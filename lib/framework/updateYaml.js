...
module.exports = async function({project, data}) {
	...
	function handleNew(diffEntry) {
		...
		changes.push({
			type: "insert",
			parentPosition,
			value: `${" ".repeat(indent)}${diffEntry.path[diffEntry.path.length - 1]}:${formatValue(diffEntry.rhs, indent)}`
		});
	}
	...
	if (!positionData.framework) {
		...
		adoptedProjectYaml = applyChanges(projectDocumentContent, changes);
	}
