...
module.exports = async function(project, newData) {
	const {fromYaml, getPosition, getValue} = require("data-with-position");

	const configPath = project.configPath || "ui5.yaml";
	const configFile = require("fs").readFileSync(configPath, {encoding: "utf8"});

	const projectConfigFile = await getProjectYamlDocument({
		project,
		configFile,
		configPath
	});

	const data = fromYaml(projectConfigFile);
	...
	const adoptedProjectYaml = applyReplacements(projectConfigFile, replacements);

	// TODO: Combine adoptedProjectYaml with other contents of yaml (other documents)
	const adoptedYaml = adoptedProjectYaml;
