...
async function createRoute(name, webappPath) {
	const manifest = JSON.parse(await readFile(webappPath + "/manifest.json", "utf8"));
	...
	if (metaInformation) {
		if (metaInformation.type == "view") {
			...
			if (!(await fsHelper.exists(`${metaInformation.webappPath}/${metaInformation.type}/${name}.${metaInformation.type}.js`) ||
				await fsHelper.exists(`${metaInformation.webappPath}/${metaInformation.type}/${name}.js`))) {
				await createComponent(namespace, metaInformation, name);
				statusMessage = metaInformation.type;
			} else {
				statusMessage = capitalizeFirstLetter(`${metaInformation.type} ${name} already exists`);
			...
			if (!await fsHelper.exists(`${metaInformation.webappPath}/${name}.js`)) {
				await createComponent(namespace, metaInformation, name);
				if (!await fsHelper.exists(`${metaInformation.webappPath}/manifest.json`)) {
