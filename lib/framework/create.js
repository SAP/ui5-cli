...
async function createTranslationFile(namespace, metaInformation) {
	...
	if (manifest["sap.app"]) {
		if (manifest["sap.app"].title) {
			content = `# App Descriptor
appTitle=${manifest["sap.app"].title}
`;
			manifest["sap.app"].title= "{{appTitle}}";
		}

		if (!metaInformation.language) {
			if (!manifest["sap.app"].i18n) {
				manifest["sap.app"].i18n = `${fileInfo.type}/${fileInfo.name}.${fileInfo.fileExtension}`;
			}
		}
	}
...
async function createManifest({namespace, project, savePath}) {
	...
	const manifest = {
		...
		"sap.app": {
			"id": `${namespace}`,
			"type": `${project.type}`,
			"title": `${capitalizeFirstLetter(project.metadata.name)}`,
			"applicationVersion": {
				"version": "1.0.0"
			}
		},
