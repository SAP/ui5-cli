...
async function createRoute(name, webappPath) {
	...
	return writeFile(webappPath + "/manifest.json", data);
}
...
module.exports = async function({name, metaInformation, project}) {
	...
	const webappPath = path.resolve(`./${project.resources.configuration.webapp}`);
	metaInformation.webappPath = webappPath;
	name = name[0].toUpperCase() + name.slice(1);
	try {
		if (metaInformation.type == "view") {
			if (!fsHelper.exists(webappPath + `/${metaInformation.type}/${name}.view.xml`)) {
			...
			if (!fsHelper.exists(webappPath + `/${metaInformation.type}/${name}.${metaInformation.type}.js`)) {
				await createController(project, metaInformation, name);
				statusMessage = metaInformation.type;
			}
		}
