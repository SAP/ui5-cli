...
async function createTranslationFile(namespace, metaInformation) {
	...
	await writeJSON(manifest, metaInformation.savePath);
	...
	if (metaInformation) {
		if (metaInformation.type == "view") {
			...
			if (!await fsHelper.exists(`${metaInformation.savePath}/${metaInformation.type}/i18n.properties`) ||
				!await fsHelper.exists(`${metaInformation.savePath}/${metaInformation.type}/i18n_${metaInformation.language}.properties`)) {
				await createTranslationFile(project.metadata.namespace, metaInformation);
				statusMessage = `Create ${metaInformation.language.toUpperCase()} translation file for project`;
			} else {
