...
function viewCommandBuilder(cli) {
	return cli
		...
		.example("$0 create view example --namespaces=sap.m", "create a view with the framework library sap.m as namespace.")
...
function controllerCommandBuilder(cli) {
	return cli
		...
		.example("$0 create controller example --modules=sap/m/MessageToast", "create a controller with the module MessageToast.");
}
...
async function executePrompts(argv, project, normalizerOptions) {
	...
	if (argv._[1] == "view") {
		...
		await executeControllerPrompts(project, argv, normalizerOptions);
	}
}
...
async function executeControllerPrompts(project, argv, normalizerOptions) {
