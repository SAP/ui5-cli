...
const {create, createComponent} = require("../../framework/create");
...
createCommand.builder = function(cli) {
	return cli
		...
		.command("component [name]", "Create Component with manifest.json", {
			handler: handleCreation,
...
async function handleCreation(argv) {
	...
	if (argv["interactive"] && !(argv["_"][1] == "component")) {
