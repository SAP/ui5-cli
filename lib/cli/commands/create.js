...
createCommand.builder = function(cli) {
	return cli
		...
		.command("bootstrap [name]", "Create bootstrap for current project", {
			handler: handleCreation,
			builder: bootstrapCommandBuilder,
			middlewares: [baseMiddleware]
		})
		.command("i18n [lang]", "Create translation file for current project", {
			handler: handleCreation,
			builder: translationCommandBuilder,
			middlewares: [baseMiddleware]
		})
		.option("interactive", {
...
function viewCommandBuilder(cli) {
	return cli
		...
		.example("$0 create view example --route", "Create a view as root.");
}
...
function customControlCommandBuilder(cli) {
	return cli
		...
		.example("$0 create control example --modules=sap/m/MessageToast", "create a control " +
...
function componentCommandBuilder(cli) {
	return cli
		...
		.example("$0 create component", "Create the root Component.")
		.example("$0 create component example --modules=sap/ui/json/JSONModel", "create a Subcomponent " +
...
function translationCommandBuilder(cli) {
	return cli
		.positional("lang", {
			describe: "Language code for translation file (empty for default)",
			type: "string"
		})
		.example("$0 create i18n", "create default translation file")
		.example("$0 create i18n en", "create translation file for english");
}
...
async function handleCreation(argv) {
	...
	if (!name && !(argv._[1] == "component" || argv._[1] == "bootstrap" || argv._[1] == "i18n")) {
...
async function executePrompts(argv, virtualFileSystemList) {
	...
	if (argv._.length == 1) {
		prompt = new Select({
			...
			choices: [
				...
				"Component", "Bootstrap", "Translation"
			]
		});
	...
	} else if (argv._[1] == "i18n") {
		await executeTranslationPrompts(argv);
	}
}
...
async function executeTranslationPrompts(argv) {
	if (!argv.lang) {
		const prompt = new Input({
			message: "Provide a language code (empty for default):"
		});
...
async function getMetaInformation(argv, webappPath, virtualFileSystemList) {
	let namespaceList = [];
	let moduleList = [];
	if (!(argv["_"][1] == "bootstrap" || argv["_"][1] == "i18n")) {
		virtualFileSystemList = await virtualFileSystemList.filter((fs) => {
			return !fs._project.type.includes("theme");
		});
