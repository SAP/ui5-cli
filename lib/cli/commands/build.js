...
build.builder = function(cli) {
	return cli
		...
		.example("ui5 build --all --exclude-task=* --include-task=minify generateComponentPreload",
			"Build project and dependencies but only apply the minify- and generateComponentPreload tasks")
		.example("ui5 build --all --include-task=minify --exclude-task=generateComponentPreload",
			"Build project and dependencies by applying all default tasks including the minify " +
