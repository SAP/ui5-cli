...
addCommand.handler = async function(argv) {
	...
	if (!yamlUpdated) {
		if (argv.config) {
			throw new Error(`Internal error while adding ${libraries.join(" ")} to config at ${argv.config}`);
		} else {
			throw new Error(`Internal error while adding ${libraries.join(" ")} to ui5.yaml`);
		}
	} else {
		console.log(`Updated configuration written to ${argv.config || "ui5.yaml"}`);
		const formattedLibraries = libraries.map(($) => $.name).join(" ");
		let logMessage = `Added framework ${libraries.length === 1 ? "library": "libraries"} ${formattedLibraries} as`;
		if (development) {
