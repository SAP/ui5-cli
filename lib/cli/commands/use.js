...
useCommand.builder = function(cli) {
	return cli.positional("framework-info", {
		describe: "Framework name, version or both. TODO",
		type: "string"
	});
};
...
useCommand.handler = async function(argv) {
	...
	if (!yamlUpdated) {
		if (argv.config) {
			console.log(`Failed to update config at ${argv.config} to ${usedFramework} version ${usedVersion}`);
		} else {
			console.log(`Failed to update ui5.yaml to ${usedFramework} version ${usedVersion}`);
		}
	} else {
		if (argv.config) {
