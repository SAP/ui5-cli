...
useCommand.handler = async function(argv) {
	...
	if (!yamlUpdated) {
		...
		process.stderr.write(`Updated configuration written to ${argv.config || "ui5.yaml"}`);
		process.stderr.write("\n");
		process.stderr.write(`This project is now using ${usedFramework} version ${usedVersion}`);
		process.stderr.write("\n");
	}
};
