	...
	return cli
		...
		.option("cache-mode", {
			describe:
				...
				"The 'Default' behavior is to invalidate the cache after 9 hours. Use 'Force' to use the cache only " +
				"(no requests are made). Use 'Off' to invalidate any existing cache",
			type: "string",
			default: "Default",
			coerce: (arg) => {
				// Ensure correct format: First letter uppercase followed by lowercase.
				return arg === "Default" ? arg : arg[0].toUpperCase() + arg.slice(1).toLowerCase();
			}
		})
