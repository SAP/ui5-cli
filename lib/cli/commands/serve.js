...
serve.handler = async function(argv) {
	const {default: ui5project} = await import("@ui5/project");
	const {generateProjectGraph} = ui5project;
	const {default: ui5Server} = await import("@ui5/server");
	const {server, sslUtil} = ui5Server;
	...
	if (serverConfig.h2) {
		const {key, cert} = await sslUtil.getSslCertificate(serverConfig.key, serverConfig.cert);
