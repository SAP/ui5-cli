...
test.serial("Writes ui5.yaml to fs with manifest, type application", async (t) => {
	const {fsHelperStub, fsWriteFileStub, consoleLogStub, safeDumpYamlStub, runStub} = t.context;
	...
	runStub.onCall(0).resolves("my.app");
...
test.serial("Writes ui5.yaml to fs, manifest exists, type library", async (t) => {
	...
	fsHelperStub.withArgs(ui5YamlPath).resolves(false);
	fsHelperStub.withArgs(`${srcPath}/manifest.json`).resolves(true);
	fsWriteFileStub.callsArgWith(2);
	safeDumpYamlStub.returns(ui5Yaml);
	sinon.stub(init, "init").resolves(projectConfig);
	const frameworkCreateManifestStub = sinon.stub(createFramework, "createManifest");
	...
	t.is(frameworkCreateManifestStub.callCount, 0, "Create Manifest function should not be called");
